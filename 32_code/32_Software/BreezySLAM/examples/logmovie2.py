...
from list_map.grid_map import ListGridMap
...
def threadfunc(robot, slam, timestamps, lidars, odometries, mapbytes, pose):
    ...
    for scanno in range(len(lidars)):
        ...
        if odometries is None:
            ...
            velocities = robot.computePoseChange(odometries[scanno]) #并非velocities, Returns a tuple (dxyMillimeters, dthetaDegrees, dtSeconds)
            #print(f"ve{velocities}")
            print(f"odo{odometries[scanno]}")
        ...
        #
        #print(f"pose[0]{pose[0]} pose[1]{pose[1]}  pose[2] {pose[2]} ")
...
def main():
    ...
    lgm = ListGridMap(MAP_SIZE_PIXELS,MAP_SIZE_METERS)
    ...
    thread = Thread(target=threadfunc,
                    args=(robot, slam, timestamps, lidars, odometries if use_odometry else None, mapbytes, pose))
    thread.daemon = True
    thread.start()
